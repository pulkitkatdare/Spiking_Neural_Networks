...
weights=zeros(1,500);
axonal=zeros(1,500);
layer_1_input=cell(1,500);
layer_1_current=cell(1,500);
layer_input=cell(1,500);
layer_output=cell(1,500);
weights_matrix=cell(1,500);
V_Response=cell(1,500);
%neuronal
lambda=100;
sum=0;
I_O=1e-12;
...
if V_t>=V_T
    for i1=j1:10000
    layer_input{i}(1,i1)=I_O*weights(i)*(exp(-(i1-j1-axonal(i)*10)/150)-exp(-(i1-j1-axonal(i)*10)/37.50))*heaviside(i1-j1-axonal(i)*10)+layer_input{i}(1,i1);
    end
    rastor{i}(1,j1)=1;
j1=j1+20;
