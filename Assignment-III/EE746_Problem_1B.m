...
I_app{2}(1,1:10)=I_A*ones(1,10);
I_app{3}=zeros(1,5000);
I_app{3}(40:49)=I_A*ones(1,10);
I_app{4}=zeros(1,5000);
I_app{4}(80:89)=I_A*ones(1,10);
V_Response=[[],[],[],[],[]];
I_syn=[[],[],[],[],[]];
for i =1 : 5
...
for t=1:5000;
I_syn{1}(t)=I_O*3000*(exp(-(t-m(1)-10)*0.1/(T_W))*heaviside(t-m(1)-10)-exp(-(t-m(1)-10)*0.1/(T_S))*heaviside(t-m(1)-10))+I_O*3000*(exp(-(t-m(2)-50)*0.1/(T_W))*heaviside(t-m(2)-50)-exp(-(t-m(2)-50)*0.1/(T_S))*heaviside(t-m(2)-50))+I_O*3000*(exp(-(t-m(3)-90)*0.1/(T_W))*heaviside(t-m(3)-90)-exp(-(t-m(3)-90)*0.1/(T_S))*heaviside(t-m(3)-90));
I_syn{5}(t)=I_O*3000*(exp(-(t-m(1)-80)*0.1/(T_W))*heaviside(t-m(1)-80)-exp(-(t-m(1)-80)*0.1/(T_S))*heaviside(t-m(1)-80))+I_O*3000*(exp(-(t-m(2)-50)*0.1/(T_W))*heaviside(t-m(2)-50)-exp(-(t-m(2)-50)*0.1/(T_S))*heaviside(t-m(2)-50))+I_O*3000*(exp(-(t-m(3)-10)*0.1/(T_W))*heaviside(t-m(3)-10)-exp(-(t-m(3)-10)*0.1/(T_S))*heaviside(t-m(3)-10));
end
...
Input=[I_app{1}(1,j),I_app{2}(1,j),I_app{3}(1,j),I_app{4}(1,j),I_app{5}(1,j)];
V_tprevious=V_t(i);
D_EPrevious=(1/C).*(-g_L*(V_t(i)-V_O(i))+Input(i)+I_syn{1}(j));
V_t(i)=V_t(i)+(1/C)*(-g_L*(V_t(i)-V_O(i))+Input(i))*del_t;
Input=[I_app{1}(1,j+1),I_app{2}(1,j+1),I_app{3}(1,j+1),I_app{4}(1,j+1),I_app{5}(1,j+1)];
D_ENew=(1/C)*((-g_L)*(V_t(i)-V_O(i))+Input(i)+I_syn{1}(j));
V_t(i)=V_tprevious+((D_EPrevious+D_ENew)*del_t/2);
...
Input=[I_app{1}(1,j),I_app{2}(1,j),I_app{3}(1,j),I_app{4}(1,j),I_app{5}(1,j)];
V_tprevious=V_t(i);
D_EPrevious=(1/C).*(-g_L*(V_t(i)-V_O(i))+Input(i)+I_syn{5}(j));
V_t(i)=V_t(i)+(1/C)*(-g_L*(V_t(i)-V_O(i))+Input(i))*del_t;
Input=[I_app{1}(1,j+1),I_app{2}(1,j+1),I_app{3}(1,j+1),I_app{4}(1,j+1),I_app{5}(1,j+1)];
D_ENew=(1/C)*((-g_L)*(V_t(i)-V_O(i))+Input(i)+I_syn{5}(j));
V_t(i)=V_tprevious+((D_EPrevious+D_ENew)*del_t/2);
